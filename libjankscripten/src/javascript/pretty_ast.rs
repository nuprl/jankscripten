...
impl Expr {
    pub fn to_doc(&self) -> D<()> {
        ...
        match self {
            ...
            MethodCall(obj, method, args, _) => obj.to_doc().append(
                D::text(".").append(D::text(method)).append(
                    D::text("(")
                        .append(D::intersperse(
...
mod test {
    ...
    fn parse_pretty_parse(js_code: &str) {
        let original_ast = parse("anon", js_code).expect("invalid test doesn't parse");
        let pretty = original_ast.to_pretty(WIDTH);
        let pretty_ast =
            parse("anon", &pretty).expect(&format!("parsing pretty-printed string: {}", &pretty));
