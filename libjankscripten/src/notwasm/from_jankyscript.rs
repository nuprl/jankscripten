...
//! ```
//!
//! The following function translats from the source to target. (Use `id` as the initial value for
//! `k`):
//!
//! ```
//! anf : expr -> (a -> e) -> e
...
//! anf (let x = e1 in e2) = anf e1 (\y -> let x = y in (anf e2 k))
//! ```
//!
//! Unfortunately, the previous function introduces a lot of useless names. For example:
//! 
//! ```
//! anf (let x = 1 + 2 in x) id = let r = 1 + 2 in (let x = r in x)
//! ```
//! 
...
impl<'a> C<'a> {
    ...
    fn recv_e(self, s: &'a mut S, e: Expr) -> Rope<Stmt> {
        match self {
            ...
            C::Id(f) => {
                let x = s.fresh();
                Rope::singleton(Stmt::Var(VarStmt::new(x.clone(), e)))
                    .append(f(s, x))
            }
            C::Atom(f) => {
                let x = s.fresh();
                Rope::singleton(Stmt::Var(VarStmt::new(x.clone(), e)))
                    .append(f(s, Atom::Id(x)))
            },
            C::Expr(f) => f(s, e),
        }
    }
...
fn compile_exprs<'a>(s : &'a mut S, 
    exprs: Vec<J::Expr>, 
    cxt: impl FnOnce(&'a mut S, Vec<Id>) -> Rope<Stmt>) -> Rope<Stmt> {
    let mut ids = Vec::<Id>::new();
    let mut stmts = Rope::new();
    for e in exprs.into_iter() {
        stmts = stmts.append(compile_expr(s, e, C::id(|s, x| {
            ids.push(x);
            Rope::Nil
        })));
    }
...
fn compile_expr<'a>(s : &'a mut S, expr: J::Expr, cxt: C<'a>) -> Rope<Stmt> {
    match expr {
        ...
        J::Expr::Binary(op, e1, e2) => compile_expr(s, *e1, C::a(move |s, a1| {
            compile_expr(s, *e2, C::a(|s, a2| {
                // TODO(arjun): compile binops correctly
                cxt.recv_a(s, Atom::Binary(BinaryOp::I32Add, Box::new(a1), Box::new(a2)))
            }))
        })),
        J::Expr::Call(fun, args) => compile_expr(s, *fun, C::id(move |s, fun_id|
...
fn compile_stmt<'a>(s: &'a mut S, stmt: J::Stmt) -> Rope<Stmt> {
    ...
    match stmt {
        S::Var(x, t, e) =>
            compile_expr(s, *e, C::e(|s, e_notwasm| {
                Rope::singleton(Stmt::Var(VarStmt::new(x, e_notwasm)))
            })),
        S::Block(stmts) => Rope::singleton(Stmt::Block(stmts.into_iter().map(|stmt| compile_stmt(s, stmt)).flatten().collect())),
...
pub fn from_jankyscript(janky_program: J::Stmt) -> Program {
    ...
    let main_body = Stmt::Block(compile_stmt(&mut state, janky_program).into_iter().collect());
    state.new_function(
        Id::from("main"), 
        Function {
            body: main_body,
            params: Vec::new(),
            fn_type: FnType { args: Vec::new(), result: None }
        });
    Program {
