...
fn type_check_stmt(env: Env, s: &mut Stmt, ret_ty: &Option<Type>) -> TypeCheckingResult<Env> {
    match s {
        ...
        Stmt::Label(_lbl, s_body, s) => {
            // LATER label checking
            type_check_stmt(env.clone(), s_body, ret_ty)?;
            Ok(env)
        }
        Stmt::Break(_lbl, s) => Ok(env),
        Stmt::Return(a, s) => {
        ...
        Stmt::Trap => Ok(env),
        Stmt::Goto(_lbl, s) => unimplemented!(),
    }
}
...
fn type_check_expr(env: &Env, e: &mut Expr) -> TypeCheckingResult<Type> {
    match e {
        ...
        Expr::Atom(a, s) => type_check_atom(env, a),
        ...
        Expr::Closure(id, _, s) => match lookup(env, id) {
...
fn type_check_atom(env: &Env, a: &mut Atom) -> TypeCheckingResult<Type> {
    match a {
        ...
        Atom::Id(id, s) => lookup(env, id),
        Atom::GetPrimFunc(id, s) => lookup(env, id),
        Atom::StringLen(a, s) => {
        ...
        Atom::EnvGet(_, ty, s) => Ok(ty.clone()),
    }
}
