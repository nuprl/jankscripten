...
fn type_check_stmt(env: Env, s: &mut Stmt, ret_ty: &Option<Type>) -> TypeCheckingResult<Env> {
    match s {
        ...
        Stmt::If(a_cond, s_then, s_else, _s) => {
        ...
        Stmt::Label(_lbl, s_body, _s) => {
            // LATER label checking
            type_check_stmt(env.clone(), s_body, ret_ty)?;
            Ok(env)
        }
        Stmt::Break(_lbl, _s) => Ok(env),
        Stmt::Return(a, _s) => {
        ...
        Stmt::Trap => Ok(env),
        Stmt::Goto(_lbl, _s) => unimplemented!(),
    }
}
...
fn type_check_expr(env: &Env, e: &mut Expr) -> TypeCheckingResult<Type> {
    match e {
        ...
        Expr::ArraySet(a_arr, a_idx, a_val, _s) => {
            let got_arr = type_check_atom(env, a_arr)?;
        ...
        Expr::HTSet(a_ht, a_field, a_val, _s) => {
        ...
        Expr::ObjectSet(a_obj, a_field, a_val, _s) => {
        ...
        Expr::ClosureCall(id_f, actuals, _s) => {
            let got_f = lookup(env, id_f)?;
        ...
        Expr::Atom(a, _s) => type_check_atom(env, a),
        ...
        Expr::Closure(id, _, _s) => match lookup(env, id) {
...
fn type_check_atom(env: &Env, a: &mut Atom) -> TypeCheckingResult<Type> {
    match a {
        ...
        Atom::Id(id, _s) => lookup(env, id),
        Atom::GetPrimFunc(id, _s) => lookup(env, id),
        Atom::StringLen(a, _s) => {
        ...
        Atom::ObjectGet(a_obj, a_field, _s) => {
            let got_obj = type_check_atom(env, a_obj)?;
            let got_field = type_check_atom(env, a_field)?;
        ...
        Atom::EnvGet(_, ty, _s) => Ok(ty.clone()),
    }
}
