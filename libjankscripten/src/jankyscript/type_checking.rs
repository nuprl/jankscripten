...
impl Into<TypeCheckingError> for String {
    fn into(self) -> TypeCheckingError {
        TypeCheckingError::Error(self)
    }
}
...
pub fn type_check(prims: Rc<HashMap<String, Type>>, stmt: &Stmt) -> TypeCheckingResult<()> {
    let mut tc = Tc { prims };
    match tc.stmt(
        stmt,
        get_global_object()
...
struct Tc {
    prims: Rc<HashMap<String, Type>>,
}


impl Tc {

    fn stmt(&mut self, stmt: &Stmt, env: Env, ret_ty: &Option<Type>) -> TypeCheckingResult<Env> {
        match stmt {
            Stmt::Empty => Ok(env),
            Stmt::Expr(e, _) => {
                self.expr(&e, env.clone())?;
                Ok(env)
            }
            Stmt::Finally(try_body, finally_body, _) => {
                // try_body should be well-typed
                self.stmt(&try_body, env.clone(), ret_ty)?;

                // finally_body should be well-typed
                self.stmt(&finally_body, env.clone(), ret_ty)?;
            ...
            Stmt::Catch(try_body, ex_name, catch_body, _) => {
                ...
                self.stmt(&catch_body, catch_body_env, ret_ty)?;

                Ok(env)
            }
            Stmt::Loop(body, _) => {
                // type check body in a new scope
                self.stmt(&body, env.clone(), ret_ty)?;
            ...
            Stmt::ForIn(bind, container, body, s) => {
                ...
                self.stmt(&body, for_in_env, ret_ty)?;
                Ok(env)
            }
            Stmt::Throw(e, _) => {
            ...
            Stmt::Label(_id, stmt, _) => {
                ...
                self.stmt(&stmt, env.clone(), ret_ty)?;
            ...
            Stmt::If(c, t, e, s) => {
                    ...
                    "if condition",
                    Type::Bool,
                    self.expr(c, env.clone())?,
                    &s,
                )?;

                self.stmt(&t, env.clone(), ret_ty)?;
                self.stmt(&e, env.clone(), ret_ty)?;

                Ok(env)
            }
            Stmt::Block(stmts, _) => {
                self.stmts(stmts, env.clone(), ret_ty)?;
                Ok(env)
            }
            Stmt::Return(e, s) => {
                let e_type = self.expr(e, env.clone())?;
    ...
    fn stmts(&mut self, stmts: &Vec<Stmt>, env: Env, ret_ty: &Option<Type>) -> TypeCheckingResult<Env> {
        let mut env = env;
        for s in stmts {
            env = self.stmt(s, env, ret_ty)?;
        }
        Ok(env)
    }
    ...
    fn func(&mut self, f: &Func, env: Env) -> TypeCheckingResult<Type> {
        let mut function_env = env.clone();
        function_env.extend(f.args_with_typs.clone().into_iter());

        self.stmt(&f.body, function_env, &Some(f.result_typ.clone()))?;
    ...
    fn expr(&mut self, expr: &Expr, env: Env) -> TypeCheckingResult<Type> {
        match expr {
            Expr::JsOp(..) => panic!("cannot type-check JsOp"),
            Expr::Func(f, _) => self.func(f, env),
            Expr::Assign(lval, rval, s) => {
                // rval should be well typed
                let rval_ty = self.expr(&rval, env.clone())?;
                ...
                match &**lval {
                    ...
                    LValue::Bracket(e, computed_prop) => {
                        ...
                        self.expr(computed_prop, env.clone())?;
            ...
            Expr::Call(fun, args, s) => {
                ...
                let fun_type = self.expr(fun, env.clone())?;

                // type check this call
                self.fn_call(fun_type, args, env, s.clone())
            }
            Expr::PrimApp(prim, args, s) => {
                let prim_ty = self.prims.get(prim)
                    .ok_or(format!("{} is not a primitive", &prim).into())?;
                self.fn_call(prim_ty.clone(), args, env, s.clone())
            }
            Expr::Coercion(coercion, e, s) => {
                // type the expression. regardless of the coercion, the expression
                // needs to be well-typed.
                let actual_type = self.expr(e, env)?;
            ...
            Expr::Dot(obj, _prop, s) => {
                let obj_type = self.expr(obj, env)?;
            ...
            Expr::Bracket(obj, dyn_prop, s) => {
                let obj_type = self.expr(obj, env.clone())?;

                ensure_indexable("brackets object", obj_type, s.clone())?;

                let dyn_prop_type = self.expr(dyn_prop, env)?;
            ...
            Expr::PrimCall(prim, args, s) => {
                ...
                self.fn_call(prim_type, args, env, s.clone())
            }
            Expr::Unary(op, e, s) => {
            ...
            Expr::Binary(op, e_l, e_r, s) => {
                ...
                let (ty_in, ty_out) = op.janky_typ();
                let got_l = self.expr(e_l, env.clone())?;
                let got_r = self.expr(e_r, env.clone())?;
            ...
            Expr::NewRef(boxed_e, boxed_t, p) => {
                    ...
                    boxed_t.clone(),
                    self.expr(&boxed_e, env)?,
                    p,
                )?;
                Ok(Type::Ref(Box::new(boxed_t.clone())))
            }
            Expr::Deref(e, t_annot, p) => match self.expr(e, env)? {
                Type::Ref(t) => {
            ...
            Expr::Store(e1, e2, t_annot, p) => match self.expr(e1, env.clone())? {
                Type::Ref(t1) => {
