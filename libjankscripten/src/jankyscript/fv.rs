...
fn var_summary(stmt: &mut Stmt) -> (IdSet, IdSet) {
    ...
    match stmt {
        ...
        Block(stmts) => {
            let (declared, referenced): (Vec<_>, Vec<_>) = stmts.iter_mut().map(|s| var_summary(s)).unzip();
            (IdSet::unions(declared.into_iter()), IdSet::unions(referenced.into_iter()))
        }
        Empty => (empty(), empty()),
        ...
        If(cond, true_part, false_part) => {
            ...
            fv_expr(cond).union(referenced_in_true).union(referenced_in_false))
        }
...
fn fv_expr(expr: &mut Expr) -> IdSet {
    ...
    match expr {
        ...
        New(e, es) | Call(e, es) => fv_expr(e).union(IdSet::unions(es.iter_mut().map(|arg| fv_expr(arg)))),
